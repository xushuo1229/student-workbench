...
import { X, LogOut, Pencil, Check, User as UserIcon, Sparkles } from 'lucide-react'
...
function UserFields({ form, setForm, editing }) {
  const inputCls = clsx('input-base', !editing && 'bg-slate-50 text-slate-500')
...
export function ProfileModal({ mode = 'manage' }) {
  const { data, profileOpen, closeProfile, loginUser, updateUser, logoutUser } = useStore()
  const isLogin = mode === 'login'
  const show = isLogin ? !data.isLoggedIn : profileOpen
  const [editing, setEditing] = useState(isLogin)
  const [form, setForm] = useState({ ...data.user })

  useEffect(() => {
    setForm({ ...data.user })
    setEditing(isLogin)
  }, [data.user, isLogin, profileOpen])
  ...
  const handleSubmit = () => {
    ...
    if (!name) {
      // 兜底：避免空白昵称
      setForm({ ...form, name: '同学' })
      loginUser({ ...form, name: '同学' })
      return
    }
    if (isLogin) loginUser(form)
    else {
      updateUser(form)
      setEditing(false)
    }
  ...
  return createPortal(
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
      <div className={clsx('absolute inset-0', isLogin ? 'bg-gradient-to-br from-brand-100/90 via-white/80 to-blue-100/90 backdrop-blur-md' : 'bg-slate-900/20 backdrop-blur-sm animate-fade-in')} />
      <div className="relative z-10 w-full max-w-lg overflow-hidden rounded-4xl border border-white/70 bg-white/95 shadow-soft animate-pop-in">
        {/* Header */}
        <div className={clsx('px-6 pt-6', isLogin && 'text-center')}>
          {isLogin ? (
            <>
              <div className="mx-auto mb-3 flex h-16 w-16 items-center justify-center rounded-3xl bg-brand-100 text-4xl shadow-glass">
                {form.avatar || '🙂'}
              </div>
              <h2 className="text-xl font-bold text-slate-800">欢迎来到自律工作台</h2>
              <p className="mt-1 text-sm text-slate-400">先完善你的个人资料，开启专属成长之旅 ✨</p>
            </>
          ) : (
        ...
        <div className="max-h-[68vh] overflow-y-auto px-6 py-5">
          {isLogin ? (
            <UserFields form={form} setForm={setForm} editing />
          ) : editing ? (
            <UserFields form={form} setForm={setForm} editing />
          ) : (
